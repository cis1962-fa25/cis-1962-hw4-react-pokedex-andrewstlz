...
function App() {
    ...
    const [idToPokemon, setIdToPokemon] = useState<Map<number, Pokemon>>(
        new Map(),
    );
    ...
    useEffect(() => {
        PokemonAPI.setToken(
            'eyJhbGciOiJIUzI1NiJ9.eyJwZW5ua2V5IjoiYW5kcmV3engiLCJpYXQiOjE3NTkwOTgyMTgsImlzcyI6ImVkdTp1cGVubjpzZWFzOmNpczE5NjIiLCJhdWQiOiJlZHU6dXBlbm46c2VhczpjaXMxOTYyIiwiZXhwIjoxNzY0MjgyMjE4fQ.Mj4Ijg8x0m_1gBv_EsO2N5bk561u9N6AXH1xt0imIBg',
        );
        loadBox();
    }, []);
    ...
    const submitCatch = async (entry: InsertBoxEntry | UpdateBoxEntry) => {
        setSubmitting(true);
        try {
            if ('pokemonId' in entry) {
                await PokemonAPI.createBoxEntry(entry as InsertBoxEntry);
            } else {
                throw new Error('UpdateBoxEntry passed to submitCatch');
            }
